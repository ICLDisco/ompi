    ...
    if (rank == 0) {
        ...
        for (i = 1; i < num_nodes; i++)
            if (OMPI_SUCCESS != (err = MCA_PML_CALL(irecv(&oversub[i], 1, MPI_INT,
                                                          nodes_roots[i], 111, comm_old,
                                                          &reqs[i - 1]))))
                return err;

        if (OMPI_SUCCESS != (err = ompi_request_wait_all(num_nodes - 1, reqs, MPI_STATUSES_IGNORE)))
            return err;
        ...
        if (rank == local_procs[0])
            if (OMPI_SUCCESS != (err = MCA_PML_CALL(send(&local_oversub, 1, MPI_INT, 0,
                                                         111, MCA_PML_BASE_SEND_STANDARD,
                                                         comm_old))))
                return err;
    }
...
#ifdef __DEBUG__
                fprintf(stdout,
                        "Oversubscribing OBJ/CORES resources =>  Rank Reordering Impossible\n");
#endif
                FALLBACK();
            }
#ifdef __DEBUG__
        ...
        if (0 == mca_topo_treematch_component.reorder_mode) {
            ...
            if (0 == rank) {
                ...
                if (true == topo->weighted) {
                    ...
                    if (OMPI_SUCCESS !=
                        (err =
                         comm_old->c_coll.coll_gather(MPI_IN_PLACE, size, MPI_DOUBLE, local_pattern,
                                                      size, MPI_DOUBLE, 0, comm_old,
                ...
                if (true == topo->weighted) {
                    ...
                    if (OMPI_SUCCESS !=
                        (err =
                         comm_old->c_coll.coll_gather(local_pattern, size, MPI_DOUBLE, NULL, 0, 0,
                                                      0, comm_old,
                                                      comm_old->c_coll.coll_gather_module)))
                        return err;
                }
            ...
            if (rank == local_procs[0]) {
                ...
                if (0 == rank) {
                    if (num_nodes > 1) {
                        ...
                        for (i = 1; i < num_nodes; i++)
                            if (OMPI_SUCCESS !=
                                (err =
                                 MCA_PML_CALL(irecv
                                              (objs_per_node + i, 1, MPI_INT, nodes_roots[i], 111,
                                               comm_old, &reqs[i - 1]))))
                                ERR_EXIT(err);
                        ...
                        for (i = 1; i < num_nodes; i++)
                            if (OMPI_SUCCESS !=
                                (err =
                                 MCA_PML_CALL(irecv
                                              (obj_mapping + displs[i], objs_per_node[i], MPI_INT,
                    ...
                    if (num_nodes > 1) {
                        ...
                        if (OMPI_SUCCESS !=
                            (err =
                             MCA_PML_CALL(send
                                          (obj_to_rank_in_comm, num_objs_in_node, MPI_INT, 0, 111,
                ...
                if (num_nodes > 1) {
                    if (rank != 0) {
                        if (OMPI_SUCCESS !=
                            (err =
                             MCA_PML_CALL(send
                                          (hierarchy, (MAX_LEVELS + 1), MPI_INT, 0, 111,
                                           MCA_PML_BASE_SEND_STANDARD, comm_old))))
                            ERR_EXIT(err);
                    } else {
                        memset(reqs, 0, (num_nodes - 1) * sizeof(MPI_Request));
                        for (i = 1; i < num_nodes; i++)
                            if (OMPI_SUCCESS !=
                                (err =
                                 MCA_PML_CALL(irecv
                                              (hierarchies + i * (MAX_LEVELS + 1), (MAX_LEVELS + 1),
                                               MPI_INT, nodes_roots[i], 111, comm_old,
                                               &reqs[i - 1])))) {
                                free(hierarchies);
                                ERR_EXIT(err);
                            }
                ...
                if (0 == rank) {
                    ...
                    for (i = 0; i < num_nodes; i++) {
                        ...
                        for (j = 1; j <= suppl; j++)
                            *(base_ptr + tm_topology->nb_levels) *=
                                *(base_ptr + tm_topology->nb_levels + j);
                    }
                    if (num_nodes > 1) {
                    ...
                    for (i = 1; i < tm_topology->nb_levels; i++)
                        tm_topology->nb_nodes[i] =
                            tm_topology->nb_nodes[i - 1] * tm_topology->arity[i - 1];
                    ...
                    for (i = 0; i < tm_topology->nb_levels; i++) {
                        tm_topology->node_id[i] =
                            (int *) calloc(tm_topology->nb_nodes[i], sizeof(int));
                    ...
                    comm_tree =
                        build_tree_from_topology(tm_opt_topology, comm_pattern, size, NULL, NULL);
            ...
            if (OMPI_SUCCESS !=
                (err =
                 ompi_comm_split(comm_old, colors[rank], ompi_process_info.my_local_rank,
                                 &localcomm, false)))
                return err;
            ...
            if (rank == local_procs[0]) {
                ...
                local_pattern =
                    (double *) calloc(num_procs_in_node * num_procs_in_node, sizeof(double));
                ...
                if (true == topo->weighted) {
                    ...
                    if (OMPI_SUCCESS !=
                        (err =
                         localcomm->c_coll.coll_gather(MPI_IN_PLACE, num_procs_in_node, MPI_DOUBLE,
                ...
                tm_opt_topology = optimize_topology(tm_topology);
                comm_tree =
                    build_tree_from_topology(tm_opt_topology, comm_pattern, num_procs_in_node, NULL,
                                             NULL);
                ...
                if (true == topo->weighted) {
                    ...
                    if (OMPI_SUCCESS !=
                        (err =
                         localcomm->c_coll.coll_gather(local_pattern, num_procs_in_node, MPI_DOUBLE,
                                                       NULL, 0, 0, 0, localcomm,
                                                       localcomm->c_coll.coll_gather_module)))
                        ERR_EXIT(err);
                }
            }
            ...
            object = hwloc_get_obj_by_depth(opal_hwloc_topology,
                                            effective_depth,
                                            matching[ompi_process_info.my_local_rank]);
            if (NULL == object)
                goto fallback;
            hwloc_bitmap_copy(set, object->cpuset);
            ...
            (void) opal_dstore.store(opal_dstore_internal,
                                     (opal_process_name_t *) ORTE_PROC_MY_NAME, &kv);
            OBJ_DESTRUCT(&kv);
            ...
            if (OMPI_SUCCESS != (err = ompi_comm_create(comm_old,
                                                        comm_old->c_local_group, newcomm))) {
                ERR_EXIT(err);
            } else {
                /* Attach the dist_graph to the newly created communicator */
                (*newcomm)->c_flags |= OMPI_COMM_DIST_GRAPH;
